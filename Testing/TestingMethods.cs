...
internal class TestingMethods
{
    ...
    private void WriteUpgrades(ICollection<SUpgradeEvent> upgradeEvents)
    {
        ...
        foreach (var e in upgradeEvents) writelist3.Add($"{e.Gameloop} {e.UpgradeTypeName} {e.PlayerId}");
    ...
    private void WriteMarinesKilled(ICollection<SUnitBornEvent> sUnitBornEvents)
    {
        ...
        foreach (var e in sUnitBornEvents)
            if (e.SUnitDiedEvent != null)
                if (e.SUnitDiedEvent.KillerUnitBornEvent != null && e.ControlPlayerId is > 0 and < 9 &&
                    e.UnitTypeName.Contains("Marine"))
                    writeList.Add(
    ...
    private void WriteInitEvents(ICollection<SUnitInitEvent> initEvents)
    {
        ...
        foreach (var e in initEvents) writeList2.Add($"{e.Gameloop} {e.UnitTypeName} {e.PlayerId}");
    ...
    private void WriteDetailsPlayers(ICollection<DetailsPlayer> detailsPlayers)
    {
        ...
        foreach (var e in detailsPlayers) writeList.Add($"{e.Name} {e.Race} {e.WorkingSetSlotId} {e.Color}");
    ...
    private void WriteBornUnits(ICollection<SUnitBornEvent> bornEvents)
    {
        ...
        foreach (var e in bornEvents)
            writeList.Add(
                $"{e.Gameloop} {ParasiteMethodHelper.ConvertIdToPlayer(e.ControlPlayerId - 1, _replay.Details.Players)} {e.UnitTypeName}");
    ...
    private void WriteTypeChangesUnits(ICollection<SUnitTypeChangeEvent> typeChanges)
    {
        ...
        foreach (var e in typeChanges)
            writeList.Add(
                $"{e.Gameloop} {ParasiteMethodHelper.ConvertIdToPlayer(e.PlayerId - 1, _replay.Details.Players)} {e.UnitTypeName}");
