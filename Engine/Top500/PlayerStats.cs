...
public class PlayerStats
{
    ...
    public PlayerStats(string handles, string playerName, double anotherPlayerKills, double killsByAnotherPlayer,
        double spawnedAmmount, double hostGames, double hostWins, double humanGames, double humaneWins,
        double survivedTimeAlienPercentages, double surviveTimeHumanPercentages)
    {
        Handles = handles;
    ...
    public static List<PlayerStats> GetPlayerStats(List<ParasiteData> parasiteDatas)
    {
        ...
        foreach (var handlesKvp in parasiteData.GameMetaData.PlayerHandles.Where(x =>
                     !handlesExclusionList.Contains(x.Key)))
        {
    ...
    private static PlayerStats CreatePlayerStatsFromParasiteData(ParasiteData parasiteData,
