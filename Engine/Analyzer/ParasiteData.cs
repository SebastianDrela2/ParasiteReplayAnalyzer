...
{
    [JsonProperty] public GameData GameData;

    [JsonProperty] public GameMetaData GameMetaData;

    [JsonProperty] public List<PlayerData> PlayerDatas = new();

    [JsonProperty] public Dictionary<string, int> PlayersKills;

    [JsonProperty] public string VictoryStatus;
    ...
    private void AddPlayerDatas(GameData gameData, IEnumerable<DetailsPlayer> players,
        ParasiteMethodHelper methodHelper)
    {
    ...
    {
        var anyAlienIsAlive = PlayerDatas.Any(x => x is { IsHost: true, IsAlive: true } or
            { IsSpawn: true, IsAlive: true });
        var anyHumanIsAlive = PlayerDatas.Any(x => x is { IsHost: false, IsSpawn: false, IsAlive: true });

        if ((anyAlienIsAlive && anyHumanIsAlive) || (!anyAlienIsAlive && !anyHumanIsAlive)) return "Undecided";

        if (anyAlienIsAlive && !anyHumanIsAlive) return "Alien Win";
