...
public class ParasiteMethodHelper
{
    ...
    public List<DetailsPlayer> GetSpawns(ICollection<SUpgradeEvent> upgradeEvents,
        ICollection<DetailsPlayer> detailsPlayers)
    {
        var spawnList = new List<DetailsPlayer>();
        var spawnUpgrades = new HashSet<string>();

        foreach (var upgradeEvent in upgradeEvents)
            if (upgradeEvent.UpgradeTypeName.Contains("isaspawn") &&
                !spawnUpgrades.Contains(upgradeEvent.UpgradeTypeName))
            {
                var colorName = upgradeEvent.UpgradeTypeName.Replace("isaspawn", "");
                var colorRgb = GetPlayerColorFromColorName(colorName);

                var player = detailsPlayers.FirstOrDefault(x =>
                    x.Color.R == colorRgb.R && x.Color.G == colorRgb.G && x.Color.B == colorRgb.B);

                if (player != null) spawnList.Add(player);

                spawnUpgrades.Add(upgradeEvent.UpgradeTypeName);
            }
    ...
    public ICollection<SUpgradeEvent> FilterUpgradeEvents(ICollection<SUpgradeEvent> upgradeEvents)
    {
        ...
        {
            if (filters != null && filters.Contains(upgrade.UpgradeTypeName)) continue;
    ...
    public Dictionary<string, int> GetPlayerKills(ICollection<DetailsPlayer> detailsPlayers,
        ICollection<SUnitBornEvent> sUnitBornEvents)
    {
        var playersKillsDict = detailsPlayers.ToDictionary(player => GetHandles(player), _ => 0);
        var unitsThatCountAsPlayerKills =
            new HashSet<string>(ReadResource("ParasiteReplayAnalyzer.Filters.UnitsThatCountAsPlayerKills.txt"));
        ...
        foreach (var sUnitBornEvent in sUnitBornEvents)
        {
            ...
            if (killerBornEvent != null)
            {
                ...
                if (player != null && unitsThatCountAsPlayerKills.Contains(sUnitBornEvent.UnitTypeName))
                    if (IsAlienOrStationSecurity(player.Name) ||
                        unitsThatCountAsPlayerKills.Contains(killerBornEvent.UnitTypeName))
    ...
    public List<DetailsPlayer> GetAlivePlayers(ICollection<DetailsPlayer> detailsPlayers,
        ICollection<SUnitBornEvent> sUnitBornEvents, List<DetailsPlayer> spawns, DetailsPlayer? alien)
    {
        var filters =
            new HashSet<string>(ReadResource("ParasiteReplayAnalyzer.Filters.UnitsThatCountAsPlayerKills.txt"));
        ...
        foreach (var sUnitBornEvent in sUnitBornEvents.Where(UnitCanBeConsideredKill))
        {
            ...
            if (player != null && filters.Contains(sUnitBornEvent.UnitTypeName))
            {
                ...
                if ((isAlien && alienFilters.Contains(sUnitBornEvent.UnitTypeName)) ||
                    spawnedList.Any(x => x.Toon.Equals(x.Toon)))
                    alivePlayers.Remove(player);
                else if (isNotAlien && !alienFilters.Contains(sUnitBornEvent.UnitTypeName)) alivePlayers.Remove(player);

                if (isAlien && !alienFilters.Contains(sUnitBornEvent.UnitTypeName)) spawnedList.Add(player);
            }
        }
    ...
    public PlayerColor GetPlayerColorFromColorName(string colorName)
    {
        ...
        if (colorMap.TryGetValue(colorName, out var playerColor)) return playerColor;
    ...
    public Dictionary<string, string> GetHandlesList(ICollection<DetailsPlayer> detailsPlayers)
    {
        ...
        foreach (var player in detailsPlayers)
        {
            ...
            if (handles != "unidentified") playerDict.Add(handles, player.Name);
        }

        return playerDict;
    }

    public Dictionary<string, double> GetLifeTimePercentagesList(ICollection<SUnitBornEvent> sUnitBornEvents,
        ICollection<DetailsPlayer> detailsPlayers, Metadata metadata)
    {
        var lifeTimeDict = new Dictionary<string, double>();
        var filters =
            new HashSet<string>(ReadResource("ParasiteReplayAnalyzer.Filters.UnitsThatCountAsPlayerKills.txt"));

        foreach (var player in detailsPlayers) lifeTimeDict.Add(GetHandles(player), 100);
    ...
    private double GetLifePercentage(double gameLoop, Metadata? metadata)
    {
        ...
        if (matchLength.Equals(0)) return 0;
    ...
    public List<DetailsPlayer?> GetSpecialRoleTeams(ICollection<DetailsPlayer> players,
        ICollection<SUpgradeEvent> upgradeEvents)
    {
        return new List<DetailsPlayer?>
        {
            ...
            new(new PlayerColor(999, 999, 999, 999), 13, 0, "", "Alien AI", 0, "AI", 0, 0, new Toon(1, "", 0, 0), 13),
            new(new PlayerColor(999, 999, 999, 999), 14, 0, "", "Station Security", 0, "AI", 0, 0,
                new Toon(2, "", 0, 0), 14)
        };
    }

    public List<DetailsPlayer> GetHumanPlayers(ICollection<DetailsPlayer> players,
        List<DetailsPlayer?> specialRoleTeams)
    {
        return players.Where(player => specialRoleTeams.All(specialRole => specialRole?.Toon != player.Toon)).ToList();
    }

    public DetailsPlayer? GetPlayerByUpgrade(string upgradeType, ICollection<SUpgradeEvent> upgradeEvents,
        ICollection<DetailsPlayer> players)
    {
        var upgradeEvent = upgradeEvents.FirstOrDefault(x => x.UpgradeTypeName.Contains(upgradeType));

        if (upgradeEvent != null) return ConvertIdToPlayer(upgradeEvent.PlayerId - 1, players);
        return null;
    }
    ...
    {
        var handles =
            $"{detailsPlayer.Toon.Region}-{detailsPlayer.Toon.ProgramId}-{detailsPlayer.Toon.Realm}-{detailsPlayer.Toon.Id}";
        handles = handles.Replace("\0", "");

        if (detailsPlayer.Toon.Id == 0) return "unidentified";
    ...
    private List<string> ReadResource(string resourceName)
    {
        ...
        while (!reader.EndOfStream)
        {
            ...
            if (line != null) returnList.Add(line);
        }
